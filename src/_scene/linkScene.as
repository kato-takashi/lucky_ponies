package _scene{
	 ...
	 * @author ...
	 */
	public class linkScene extends SceneObject {
		
		/**
		 * 新しい IndexScene インスタンスを作成します。
		 */
		 private var _up_bg:up_bg;
		 private var _Link_txt:Link_txt;
		 private var _xml_nodeNum:uint;
		 
		public function linkScene() {
			// シーンタイトルを設定します。
			title = "Link _ LuckyPonies Web _ Kato & Kato Design.";
		}
		...
		override protected function atSceneInit():void {
			addCommand(
					   ...
					   new AddChild(_up_bg, _Link_txt),
					   [new DoTweener( _Link_txt , {  alpha:1, y:0 , time:.2 , transition:"easeOutCirc" } ),
					   new DoTweener( IndexScene(parent)._arr_Btn_Thumbnail[3] , {  alpha:1 , time:.2 , transition:"easeOutCirc" } )] 
			);
		}
		...
		override protected function atSceneGoto():void {
			addCommand(
					   [new DoTweener( _Link_txt , {  alpha:0 ,y:-300 , time:.2 , transition:"easeOutCirc" } ),
						  
					   new DoTweener( IndexScene(parent)._arr_Btn_Thumbnail[3] , {  alpha:0 , time:.2 , transition:"easeOutCirc" } )],
					   new RemoveChild(container,_up_bg),
					   new RemoveChild(_up_bg, _Link_txt),
